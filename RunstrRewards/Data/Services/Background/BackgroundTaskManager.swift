...
class BackgroundTaskManager {
    ...
    // WorkoutRewardCalculator removed - rewards now come from team prize pools
    ...
    func scheduleWorkoutSync() {
        ...
        // Schedule reasonably - every 2 hours for optimal battery life
        request.earliestBeginDate = Date(timeIntervalSinceNow: 2 * 3600) // 2 hours
        
        do {
            try BGTaskScheduler.shared.submit(request)
            print("🔋 BackgroundTaskManager: Workout sync scheduled (2h)")
        } catch {
    ...
    func scheduleRewardCheck() {
        ...
        request.earliestBeginDate = Date(timeIntervalSinceNow: 1 * 3600) // 1 hour for reward updates
        
        do {
            try BGTaskScheduler.shared.submit(request)
            print("🔋 BackgroundTaskManager: Reward check scheduled (1h)")
        } catch {
    ...
    private func performQueueBasedWorkoutSync() async -> Bool {
        ...
        do {
            ...
            guard await healthKitService.checkAuthorizationStatus() else {
            ...
            let deduplicatedWorkouts = await healthKitService.detectDuplicates(in: rawWorkouts)
            ...
            let workouts = await healthKitService.detectCrossPlatformDuplicates(deduplicatedWorkouts, existingWorkouts)
                ...
                for workout in workouts {
                    ...
                    guard shouldContinueOperation() else {
                        ...
                        for remainingWorkout in workouts.dropFirst(queuedCount) {
                            let supabaseWorkout = await healthKitService.convertToSupabaseWorkout(remainingWorkout, userId: userId)
                            WorkoutSyncQueue.shared.queueWorkout(supabaseWorkout)
                        }
                        break
                    }
                    
                    let supabaseWorkout = await healthKitService.convertToSupabaseWorkout(workout, userId: userId)
    ...
    private func calculateSimpleWorkoutPoints(_ workout: HealthKitWorkout) -> Int {
        // Simple scoring for competition tracking (not Bitcoin rewards)
        ...
        if workout.workoutType.lowercased().contains("running") || workout.workoutType.lowercased().contains("cycling") {
        ...
        return min(points, 100) // Cap at 100 points for competition fairness
    }
    ...
    private func processWorkoutInForeground(_ workout: Workout, userId: String, teamMultiplier: Double) async {
        ...
        guard let healthKitWorkout = workout.toHealthKitWorkout() else {
            print("❌ BackgroundTaskManager: Failed to convert workout to HealthKit format")
            return
        }
        
        do {
            // Validate workout using anti-cheat system
            let validation = await AntiCheatService.shared.validateWorkout(
                HKWorkout(activityType: healthKitWorkout.activityType, 
                         start: healthKitWorkout.startDate, 
                         end: healthKitWorkout.endDate)
            )
            
            guard validation.isValid else {
                print("⚠️ BackgroundTaskManager: Workout \(workout.id) failed validation")
                return
            }
            
            // Simple points for competition tracking (not Bitcoin rewards)
            let basePoints = calculateSimpleWorkoutPoints(healthKitWorkout)
            let adjustedPoints = Int(Double(basePoints) * teamMultiplier)
            
            // Process competition updates
            try await CompetitionDataService.shared.updateUserProgress(
                userId: userId,
                workout: healthKitWorkout,
                points: adjustedPoints
            )
            
            print("✅ BackgroundTaskManager: Processed workout \(workout.id) - \(adjustedPoints) points")
            
        } catch {
            print("❌ BackgroundTaskManager: Failed to process workout \(workout.id): \(error)")
        }
    }
