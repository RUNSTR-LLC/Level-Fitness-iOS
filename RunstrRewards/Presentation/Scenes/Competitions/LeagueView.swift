...
class LeagueView: UIView {
    ...
    private func updatePrizePoolDisplay() {
        if let league = currentLeague {
            ...
            let payoutType = PayoutType.allCases.first { payoutType in
                let payoutDict = Dictionary(uniqueKeysWithValues: payoutType.percentages.enumerated().map { (index, percentage) in
                    (String(index + 1), Double(percentage))
                })
                return payoutDict == league.payoutPercentages
            } ?? .winnerTakesAll
    ...
    private func calculatePotentialPrize(for user: LeaderboardUser) -> Int {
        ...
        let rank = user.rank
        guard let payoutPercentages = league.payoutPercentages else {
            return 0 // No payout structure defined
        }
        ...
        // Calculate prize amount using rank as string key
        let rankKey = String(rank)
        guard let percentage = payoutPercentages[rankKey] else {
            return 0 // No percentage defined for this rank
        }
        let prizeAmount = Int(Double(teamWalletBalance) * percentage / 100.0)
