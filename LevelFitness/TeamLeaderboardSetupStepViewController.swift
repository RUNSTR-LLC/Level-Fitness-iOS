...
class TeamLeaderboardSetupStepViewController: UIViewController {
    ...
    private func setupSelectionButtons() {
        ...
        for (index, leaderboardType) in LeaderboardType.allCases.enumerated() {
            ...
            if let previousButton = previousTypeButton {
                ...
                button.topAnchor.constraint(equalTo: typeLabel.bottomAnchor, constant: 16).isActive = true
            }
        ...
        for (index, period) in LeaderboardPeriod.allCases.enumerated() {
            ...
            if let previousButton = previousPeriodButton {
                ...
                button.topAnchor.constraint(equalTo: periodLabel.bottomAnchor, constant: 16).isActive = true
            }
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            periodSection.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -24),
        ...
        // Add bottom constraints for sections
        if let lastTypeButton = typeButtons.last {
            typeSection.bottomAnchor.constraint(equalTo: lastTypeButton.bottomAnchor, constant: 16).isActive = true
        }
        
        if let lastPeriodButton = periodButtons.last {
            periodSection.bottomAnchor.constraint(equalTo: lastPeriodButton.bottomAnchor, constant: 16).isActive = true
        }
    }
    
    private func loadExistingData() {
        // Select current leaderboard type
        if let typeIndex = LeaderboardType.allCases.firstIndex(of: teamData.leaderboardType) {
    ...
    @objc private func leaderboardTypeSelected(_ sender: UIButton) {
        let selectedType = LeaderboardType.allCases[sender.tag]
