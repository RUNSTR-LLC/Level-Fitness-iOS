...
class PrizeDistributionViewController: UIViewController {
    ...
    private func setupCustomAmountInputs() {
        ...
        // Get placeholder member names since TeamData doesn't have memberNames property
        let teamMembers = (1...teamData.members).map { "Team Member \($0)" }
    ...
    @objc private func calculateDistribution() {
        ...
        Task {
            let result = await distributionService.createDistribution(
            ...
            case .success(let distribution):
                await MainActor.run {
                    ...
                    self.updateTotalVerification()
                    self.updateExecuteButtonState()
                    self.showStatus("Distribution calculated successfully", color: UIColor.systemGreen)
                }
                
            case .failure(let error):
                await MainActor.run {
                    self.showError("Failed to calculate distribution: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    private func performDistributionExecution(distribution: PrizeDistribution) {
        ...
        Task {
            let result = await distributionService.executeDistribution(distributionId: distribution.distributionId)
        
            switch result {
            case .success:
                await MainActor.run {
                    self.showStatus("Distribution executed successfully! 🎉", color: UIColor.systemGreen)
                }
            ...
            case .failure(let error):
                await MainActor.run {
                    self.showError("Distribution execution failed: \(error.localizedDescription)")
