...
extension CompetitionEvent {
    ...
    var eventType: CompetitionEventType { 
        return CompetitionEventType.fromString(type)
    }
...
enum CompetitionEventType {
    ...
    static func fromString(_ typeString: String) -> CompetitionEventType {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    print("🏆 RunstrRewards: Loaded \(fetchedEvents.count) events from Supabase")
                }
            } catch {
                print("🏆 RunstrRewards: Error fetching events: \(error)")
                await MainActor.run {
