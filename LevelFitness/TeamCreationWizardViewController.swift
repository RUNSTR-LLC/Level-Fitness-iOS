...
class TeamCreationWizardViewController: UIViewController {
    ...
    private func showStep(_ step: Int) {
        ...
        print("🧙‍♂️ TeamCreationWizard: Showing step \(step + 1) of \(totalSteps)")
    }
    ...
    private func updateNavigationButtons() {
        ...
        if currentStep == totalSteps - 1 {
            ...
            nextButton.backgroundColor = UIColor(red: 0.2, green: 0.8, blue: 0.2, alpha: 1.0) // Green for final step
        } else {
            nextButton.setTitle("Next", for: .normal)
    ...
    private func validateCurrentStep() -> Bool {
        // In full implementation, each step would validate its own data
        ...
        Task {
            do {
                ...
                let newTeam = Team(
                    ...
                    captainId: AuthenticationService.shared.currentUserId ?? "",
                    memberCount: 0,
                    totalEarnings: 0.0,
                    imageUrl: nil,
                    createdAt: Date()
...
class TeamCreationData: ObservableObject {
    ...
    @Published var leaderboardType: LeaderboardType = .distance
