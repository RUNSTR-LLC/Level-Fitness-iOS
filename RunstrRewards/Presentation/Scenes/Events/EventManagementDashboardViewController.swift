...
class EventManagementDashboardViewController: UIViewController {
    ...
    private func loadEvents() {
        print("🎛️ EventDashboard: Loading events from database for team: \(teamData.id)")
        
        Task {
            do {
                // Load real events from CompetitionDataService
                let teamEvents = try await CompetitionDataService.shared.fetchTeamEvents(teamId: teamData.id)
                
                await MainActor.run {
                    self.events = teamEvents
                    
                    print("🎛️ EventDashboard: Loaded \(self.events.count) events successfully")
                    self.updateEventsList()
                }
            } catch {
                print("🎛️ EventDashboard: Failed to load events: \(error)")
                ...
                await MainActor.run {
                    ...
                    self.showErrorMessage("Unable to load events. Please check your connection and try again.")
                }
            }
        }
    }
    
    private func eventStatusFromString(_ status: String) -> EventStatus {
    ...
    private func updateStats() {
        ...
        let totalParticipants = events.reduce(0) { $0 + $1.participantCount }
        let totalEarnings = events.reduce(0) { $0 + $1.prizePool }
...
extension EventManagementDashboardViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        eventDetailVC.onRegistrationComplete = { [weak self] success in
            // Refresh events list if registration status changed
            if success {
                self?.loadEvents()
            }
        }
