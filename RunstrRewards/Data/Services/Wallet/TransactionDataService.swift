...
class TransactionDataService {
    ...
    func getTeamWallet(teamId: String) async throws -> TeamWallet? {
        print("TransactionDataService: Getting team wallet for team \(teamId)")
        ...
        do {
            ...
            let teamWallet = response.first
            print("TransactionDataService: Retrieved team wallet: \(teamWallet?.id ?? "none")")
            return teamWallet
            
        } catch {
            print("TransactionDataService: Failed to get team wallet: \(error)")
            throw error
        }
    }
    
    func getTeamWalletsCreatedBy(captainId: String, since: Date) async throws -> [TeamWallet] {
        print("TransactionDataService: Getting team wallets created by captain \(captainId) since \(since)")
        ...
        do {
            ...
            print("TransactionDataService: Retrieved \(response.count) team wallets created by captain")
            return response
            
        } catch {
            print("TransactionDataService: Failed to get team wallets created by captain: \(error)")
            throw error
        }
    }
    ...
    func storeSubscriptionData(_ subscriptionData: SubscriptionData) async throws {
        ...
        let databaseSubscription = DatabaseSubscription(
            ...
            productId: subscriptionData.priceId,
            purchaseDate: iso8601Formatter.string(from: subscriptionData.createdAt),
            expirationDate: iso8601Formatter.string(from: subscriptionData.currentPeriodEnd),
            status: subscriptionData.status,
            originalTransactionId: subscriptionData.id,
            createdAt: iso8601Formatter.string(from: Date()),
            updatedAt: iso8601Formatter.string(from: Date())
        )
