...
// This service references models from SupabaseService, HealthKitService, ErrorHandlingService
...
class WorkoutDataService {
    ...
    func fetchUserWorkouts(userId: String, limit: Int = 20) async throws -> [Workout] {
        return try await fetchWorkouts(userId: userId, limit: limit)
    }
    ...
    private func sendWorkoutCompletionNotification(userId: String, workoutType: String) async {
        do {
            ...
            if let primaryTeam = teams.first {
                ...
                await MainActor.run {
                    Task {
                        try await NotificationService.shared.scheduleAndStoreNotification(
                            identifier: "workout_sync_\(UUID().uuidString)",
                            title: "Workout Complete! 🏃‍♂️",
                            body: "Your \(workoutType.capitalized) workout was synced to Team \(primaryTeam.name)! Contributing to team leaderboards 💪",
                            teamBranding: TeamBranding(teamId: primaryTeam.id, teamName: primaryTeam.name, teamColor: "#007AFF", teamLogoUrl: primaryTeam.imageUrl),
                            userInfo: [
                ...
                await MainActor.run {
                    Task {
                        try await NotificationService.shared.scheduleAndStoreNotification(
                            identifier: "workout_generic_\(UUID().uuidString)",
                            title: "Workout Complete! 🏃‍♂️",
                            body: "Your \(workoutType.capitalized) workout has been recorded",
                            teamBranding: nil,
                            userInfo: [
                                "workout_type": workoutType,
                                "user_id": userId
            ...
            await MainActor.run {
                Task {
                    try await NotificationService.shared.scheduleAndStoreNotification(
                        identifier: "workout_error_\(UUID().uuidString)",
                        title: "Workout Complete! 🏃‍♂️",
                        body: "Your \(workoutType.capitalized) workout has been recorded",
                        teamBranding: nil,
                        userInfo: [
