...
class EventManagementCell: UITableViewCell {
    ...
    func configure(with event: EventData) {
        ...
        eventTypeLabel.text = event.type.displayName.uppercased()
        
        // Configure status badge
        statusLabel.text = event.status.displayName.uppercased()
        statusBadge.backgroundColor = event.status.color
        
        // Configure participants
        participantsLabel.text = "\(event.participants)"
        
        // Configure prize pool
        if event.prizePool > 0 {
            prizePoolLabel.text = "\(Int(event.prizePool))"
        } else {
