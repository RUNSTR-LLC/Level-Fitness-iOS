    ...
    private func schedulePrizeCalculation(flow: CompetitionFlow) {
        if flow.prizeDistributionEnabled {
            ...
            Task {
                ...
                await self.calculateAndDistributePrizes(flow: flow)
            }
        }
    }
    
    private func calculateAndDistributePrizes(flow: CompetitionFlow) async {
        ...
        case .success(let distribution):
            ...
            let executionResult = await distributionService.executeDistribution(distributionId: distribution.distributionId)
