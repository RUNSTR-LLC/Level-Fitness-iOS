...
class ErrorHandlingService {
    ...
    private let errorCountQueue = DispatchQueue(label: "com.runstrrewards.error.count", attributes: .concurrent)
    ...
    private func updateErrorCount(context: String, error: AppError) {
        // Create a safe key using simple string concatenation with sanitization
        let sanitizedContext = context.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "")
        let safeKey = "\(sanitizedContext)_\(error.description)"
        
        // Use concurrent queue with barrier for thread-safe write
        errorCountQueue.async(flags: .barrier) { [weak self] in
            self?.errorCount[safeKey, default: 0] += 1
        }
    }
    ...
    func shouldRetry(context: String, error: AppError) -> Bool {
        // Check error count to implement proper retry logic
        let currentCount = getErrorCount(context: context, error: error)
        
        switch error {
        case .networkUnavailable, .serviceUnavailable:
            return currentCount < maxRetryAttempts
        case .authenticationRequired, .dataCorrupted:
    ...
    func resetRetryCount(context: String, error: AppError) {
        ...
        errorCountQueue.async(flags: .barrier) { [weak self] in
            self?.errorCount.removeValue(forKey: safeKey)
        }
    }
