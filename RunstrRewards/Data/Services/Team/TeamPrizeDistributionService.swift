...
class TeamPrizeDistributionService {
    ...
    private func initializeTeamWallets() {
        // Team wallets will be loaded dynamically when needed
        // No initialization of sample data required
        print("TeamPrizeDistributionService: Initialized with empty wallet cache")
    }
        ...
        for attempt in 1...maxRetryAttempts {
            ...
            if attempt < maxRetryAttempts {
                ...
                do {
                    ...
                    print("💰 Distribution: Task.sleep interrupted: \(error)")
                    // Continue with next attempt even if sleep is interrupted
                }
            }
        }
    ...
    private func sendPrizeNotification(to userId: String, amount: Double, teamName: String, reason: String) {
        Task {
            await NotificationService.shared.schedulePrizeDistributionNotification(
                userId: userId,
                eventId: "team_payout",
                prizeAmount: Int(amount),
                position: 1,
    ...
    func distributeTeamRewards(teamId: String) async throws -> DistributionResult {
        do {
            let distributions = try await getPendingDistributions(teamId: teamId)
            ...
            let recipientCount = distributions.count
            
            for distribution in distributions {
                totalAmount += Int(distribution.amount)
                // This would normally process the distribution
                // For now, mark all as successfully distributed
                distributedAmount += Int(distribution.amount)
            }
