...
extension RealtimeLeaderboardService {
    ...
    func startMockUpdates(for eventId: String) {
        // For development/testing - simulate realistic leaderboard changes
        Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { [weak self] timer in
            ...
            guard self.isEventActive(eventId: eventId) else {
                timer.invalidate()
                return
            }
            
            // Create mock update
            let mockEntries = self.generateMockLeaderboard(eventId: eventId)
            let mockUpdate = LiveLeaderboardUpdate(
                eventId: eventId,
                timestamp: Date(),
                updateType: .valueUpdate,
                affectedEntries: mockEntries,
                metadata: ["mock": true]
            )
            
            self.liveUpdates[eventId] = mockUpdate
            self.broadcastUpdate(mockUpdate)
        }
    }
    
    private func generateMockLeaderboard(eventId: String) -> [LeaderboardEntry] {
        let usernames = ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Henry"]
        ...
        return usernames.enumerated().map { index, username in
            LeaderboardEntry(
                ...
                workoutCount: Int.random(in: 5...25),
                totalDistance: Double.random(in: 1000...50000)
            )
