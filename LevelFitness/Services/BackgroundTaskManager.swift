...
class BackgroundTaskManager {
    ...
    private let workoutSyncTaskId = "com.levelfitness.workout-sync"
    private let rewardCheckTaskId = "com.levelfitness.reward-check"
    private let notificationUpdateTaskId = "com.levelfitness.notification-update"
    ...
    private func handleWorkoutSyncTask(_ task: BGProcessingTask) {
        ...
        Task {
            do {
                ...
                print("BackgroundTaskManager: Workout sync error: \(error)")
                task.setTaskCompleted(success: false)
            }
        }
    }
    ...
    private func handleRewardCheckTask(_ task: BGAppRefreshTask) {
        ...
        Task {
            do {
                ...
                print("BackgroundTaskManager: Reward check error: \(error)")
                task.setTaskCompleted(success: false)
            }
        }
    }
    ...
    private func handleNotificationUpdateTask(_ task: BGAppRefreshTask) {
        ...
        Task {
            do {
                ...
                print("BackgroundTaskManager: Notification update error: \(error)")
                task.setTaskCompleted(success: false)
            }
        }
    }
    ...
    private func performWorkoutSync() async -> Bool {
        ...
        do {
            ...
            let workouts = try await healthKitService.fetchWorkoutsSince(lastSyncDate, limit: 50)
            print("BackgroundTaskManager: Found \(workouts.count) new workouts")
