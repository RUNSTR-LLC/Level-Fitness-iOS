...
class LiveLeaderboardView: UIView {
    ...
    private func getCurrentEventData(eventId: String) -> EventData? {
        // Try to get event data from the event cache or create a basic structure
        // This is used for UI presentation when full event details aren't critical
        ...
        return EventData(
            ...
            endDate: Date().addingTimeInterval(3600),    // Ends in 1 hour
            participants: leaderboardEntries.count,
            prizePool: 0, // Will be updated when connected to real event data
            entryFee: 0
        )
    }
