...
class TeamInvitationService {
    ...
    private func storeInviteCode(teamId: String, code: String) async {
        // Store invite code in Supabase with expiration (7 days from now)
        print("Storing invite code \(code) for team \(teamId)")
        
        guard let userSession = authService.loadSession() else {
            print("TeamInvitationService: No user session found for storing invite code")
            return
        }
        
        do {
            let expirationDate = Calendar.current.date(byAdding: .day, value: 7, to: Date())
            try await supabaseService.storeTeamInviteCode(
                teamId: teamId,
                code: code,
                createdBy: userSession.id,
                expiresAt: expirationDate
            )
            print("TeamInvitationService: Successfully stored invite code")
        } catch {
            print("TeamInvitationService: Failed to store invite code: \(error)")
        }
    }
    ...
    private func validateInviteCode(_ teamId: String, code: String) async -> Bool {
        ...
        do {
            let isValid = try await supabaseService.validateTeamInviteCode(teamId: teamId, code: code)
            
            if isValid {
                // Increment the usage count since the code was successfully used
                try await supabaseService.incrementInviteCodeUsage(teamId: teamId, code: code)
            }
            
            return isValid
            
        } catch {
            print("TeamInvitationService: Error validating invite code: \(error)")
            return false
        }
    }
    ...
    func getTeamInvitations(teamId: String) async throws -> [TeamInvitationDetails] {
        return try await supabaseService.getTeamInvitations(teamId: teamId)
    }
    
    func revokeInvitation(teamId: String, invitationId: String) async throws {
        try await supabaseService.deactivateTeamInvitation(teamId: teamId, invitationId: invitationId)
    }
    
    func createNewInvitation(for teamId: String, expiresInDays: Int = 7, maxUses: Int? = nil) async throws -> String {
        // Generate a unique invite code
        let inviteCode = UUID().uuidString.prefix(8).uppercased()
        
        guard let userSession = authService.loadSession() else {
            throw NSError(domain: "TeamInvitationService", code: 401, userInfo: [NSLocalizedDescriptionKey: "User not authenticated"])
        }
