...
protocol TeamSubscriptionStatusViewDelegate: AnyObject {
...
class TeamSubscriptionStatusView: UIView {
    ...
    // MARK: - Properties
    weak var delegate: TeamSubscriptionStatusViewDelegate?
    ...
    func updateSubscriptionStatus(isSubscribed: Bool? = nil) {
        guard let teamId = teamId else { return }
        
        Task {
            // Use provided subscription status if available, otherwise check
            let isTeamSubscribed = isSubscribed ?? SubscriptionService.shared.isSubscribedToTeam(teamId)
            ...
            await MainActor.run {
                ...
                self.updateForSubscriptionTier(subscriptionStatus.subscriptionStatus)
            }
        }
    }
    ...
    private func updateForSubscriptionTier(_ tier: SubscriptionStatus) {
        ...
        case .member:
            if isSubscribed {
                descriptionLabel.text = "You're subscribed to this team and can compete for Bitcoin rewards."
                statusLabel.text = "Team Member"
                statusIconView.image = UIImage(systemName: "person.fill")
                statusIconView.tintColor = IndustrialDesign.Colors.primaryText
                statusLabel.textColor = IndustrialDesign.Colors.primaryText
            }
        case .user:
