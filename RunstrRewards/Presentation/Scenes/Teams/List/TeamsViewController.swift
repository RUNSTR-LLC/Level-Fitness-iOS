...
class TeamsViewController: UIViewController {
    ...
    @objc private func createTeamTapped() {
        ...
        Task {
            ...
            await MainActor.run {
                if subscriptionStatus.isActive && subscriptionStatus.productId == "captain_monthly_subscription" {
    ...
    private func showCreatorSubscriptionPrompt() {
        ...
        let price = SubscriptionService.shared.getCaptainSubscriptionProduct()?.displayPrice ?? "$19.99"
    ...
    private func purchaseCreatorSubscription() {
        ...
        Task {
            do {
                let success = try await SubscriptionService.shared.purchaseCaptainSubscription()
...
struct TeamData {
    ...
    let members: Int
    let memberCount: Int // Alias for members
    let prizePool: String
    let currentPrizePool: Double // Numeric version of prizePool
    let activities: [String]
    let isJoined: Bool
    
    // Convenience initializer to maintain backward compatibility
    init(id: String, name: String, description: String? = nil, captain: String, captainId: String, members: Int, prizePool: String, currentPrizePool: Double = 0, activities: [String], isJoined: Bool) {
        self.id = id
        self.name = name
        ...
        self.memberCount = members // Same value as members
