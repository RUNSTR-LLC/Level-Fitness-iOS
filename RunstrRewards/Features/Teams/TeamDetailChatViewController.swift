        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    print("🏗️ RunstrRewards: Loaded \(messageDataArray.count) team messages from Supabase")
                }
                ...
                SupabaseService.shared.subscribeToTeamChat(teamId: teamId) { [weak self] newMessage in
                    ...
                    DispatchQueue.main.async {
                        ...
                        print("🏗️ RunstrRewards: New real-time team message: \(newMessage.message)")
                    }
                }
                
            } catch {
                print("🏗️ RunstrRewards: Error fetching team messages: \(error)")
                await MainActor.run {
                    displayMessages([])
                }
            }
...
extension TeamDetailChatViewController: MessageInputViewDelegate {
    func didSendMessage(_ message: String) {
        print("🏗️ RunstrRewards: Sending message: \(message)")
        
        guard let userSession = AuthenticationService.shared.loadSession() else {
            print("🏗️ RunstrRewards: No user session found for sending message")
            return
        }
        ...
        Task {
            do {
                ...
                print("🏗️ RunstrRewards: Message sent successfully")
                ...
                print("🏗️ RunstrRewards: Error sending message: \(error)")
            }
        }
    }
}